...
#define THRESHOLD_ACC 19500 // Accelerometer threshold for earthquake detection
#define THRESHOLD_WATER 40 // Water level threshold for flood detection
...
void loop() {
  ...
  if (water_level > THRESHOLD_WATER) {
    ...
    // Check for flood
  if (water_level2 > THRESHOLD_WATER) {
    // Flood detected, sound the buzzer
    digitalWrite(BUZZER_PIN, HIGH);
    delay(1000); // Buzzer duration
    digitalWrite(BUZZER_PIN, LOW);
  ...
  Serial.println("%");
   ...
   delay(100);// Adjust delay according to the desired sampling rate
}
